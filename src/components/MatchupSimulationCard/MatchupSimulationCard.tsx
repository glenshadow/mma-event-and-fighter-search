...
export default function MatchupSimulationCard({
  ...
  return (
    <section aria-label="Predictive Matchup Simulation Model" className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-white/10 pb-4">
        <div>
          <div className="flex items-center gap-2 text-amber-500 font-mono text-[10px] font-bold uppercase tracking-widest">
            <Sparkles className="w-3.5 h-3.5" aria-hidden="true" /> Datalogic Simulation Model
          </div>
          <h4 className="text-sm font-black italic text-white uppercase tracking-tight mt-1">
            Predictive Matchup Simulation
          </h4>
        </div>
        <div className="text-[10px] font-mono text-zinc-300 dark:text-white/80 uppercase bg-black/30 border border-white/5 px-2 py-1 rounded max-w-xs leading-normal">
          🔮 Computed strictly via pre-fight variables. Independent of real outcome.
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center" aria-live="polite" role="region" aria-label="Simulated Probabilities">
        ...
        <div className="bg-white/[0.02] border border-white/10 rounded-xl p-4 flex items-center justify-between relative overflow-hidden gap-4" id="prediction-card-fighter-a">
          <div className={`absolute top-0 right-0 w-24 h-24 ${themeA.bgGlowHalf} rounded-full blur-3xl pointer-events-none`} aria-hidden="true" />
          
          {/* Headshot on the LEFT */}
          <button 
            onClick={() => onSelectFighter(fighterA.id)}
            className="shrink-0 relative z-10 cursor-pointer hover:scale-105 active:scale-95 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 rounded-full"
            id="prediction-headshot-fighter-a"
            title={`View ${fighterA.fullName} profile`}
            aria-label={`View ${fighterA.fullName} profile`}
          >
            {headshotA ? (
              <ImageWithLoader 
                src={headshotA} 
          ...
          <div className="flex flex-col items-end text-right relative z-10 flex-1">
            ...
            <span className="text-[10px] font-mono text-zinc-300 dark:text-white/80 uppercase tracking-wider mt-1">
        ...
        <div className="bg-white/[0.02] border border-white/10 rounded-xl p-4 flex items-center justify-between relative overflow-hidden gap-4" id="prediction-card-fighter-b">
          <div className={`absolute top-0 left-0 w-24 h-24 ${themeB.bgGlowHalf} rounded-full blur-3xl pointer-events-none`} aria-hidden="true" />
          ...
          <button 
            ...
            className="shrink-0 relative z-10 order-2 cursor-pointer hover:scale-105 active:scale-95 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 rounded-full"
            id="prediction-headshot-fighter-b"
            title={`View ${fighterB.fullName} profile`}
            aria-label={`View ${fighterB.fullName} profile`}
          >
            {headshotB ? (
              <ImageWithLoader 
                src={headshotB} 
      ...
      <div className="space-y-1.5">
        <div 
          role="meter"
          aria-label={`Win probability: ${fighterA.fullName} ${prediction.probA}% vs ${fighterB.fullName} ${prediction.probB}%`}
          aria-valuenow={prediction.probA}
          aria-valuemin={0}
          aria-valuemax={100}
          className="h-2 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden flex"
        >
          <div 
        ...
        <div className="flex justify-between items-center text-[10px] font-mono text-zinc-300 dark:text-white/80 uppercase tracking-widest">
      ...
      <div className="pt-2">
        <button 
          ...
          aria-controls="factor-analysis-breakdown"
          className="w-full py-2 px-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] sm:text-xs font-mono font-bold uppercase tracking-wider text-zinc-200 hover:text-white transition-colors flex items-center justify-center gap-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
        >
          <span>{showBreakdown ? 'Hide Simulated Factor Analysis' : 'Expand Simulated Factor Analysis'}</span>
          <span className="text-amber-500" aria-hidden="true">{showBreakdown ? '▲' : '▼'}</span>
        </button>

        {showBreakdown && (
          <div id="factor-analysis-breakdown" role="log" aria-label="Simulated Factor Analysis Log" className="border border-slate-200 dark:border-white/10 rounded-xl mt-3 overflow-hidden bg-black/45 divide-y divide-slate-300 dark:divide-white/5">
                ...
                <div key={i} className="p-3 sm:p-4 space-y-2">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-[10px] text-zinc-300 dark:text-white/80 block font-mono leading-none mt-1">
                        {b.desc}
                      </span>
                    </div>
                    <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${
                      favA 
                        ...
                        : 'bg-white/5 text-zinc-300 dark:text-white/80 border border-white/10'
                    }`}>
                  ...
                  <div className="grid grid-cols-3 gap-2 items-center text-center font-mono text-[10px]">
                    {/* Redcorner metric */}
                    <div className={`text-right ${favA ? `${themeA.badgeText} font-bold` : 'text-zinc-300 dark:text-white/70'}`}>
                      {b.valA}
                      <span className="text-[10px] text-zinc-400 dark:text-white/70 block mt-0.5">
                        {b.scoreA > 0 ? `+${b.scoreA.toFixed(1)} pts` : b.scoreA < 0 ? `${b.scoreA.toFixed(1)} pts` : '0.0 pts'}
                      </span>
                    </div>

                    {/* Mid-label line */}
                    <div className="h-[1px] bg-slate-400 dark:bg-white/20 relative factor-mid-line" aria-hidden="true">
                    ...
                    {/* Bluecorner metric */}
                    <div className={`text-left ${favB ? `${themeB.badgeText} font-bold` : 'text-zinc-300 dark:text-white/70'}`}>
                      {b.valB}
                      <span className="text-[10px] text-zinc-400 dark:text-white/70 block mt-0.5">
